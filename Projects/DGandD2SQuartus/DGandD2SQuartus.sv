...
module DGandD2SQuartus (Clock, Button_Up, Button_Reset, Button_Down, Button_Sign, Indicators, Segments, SigneBit);
  parameter Size = 5,
            ClockPeriod_ns = 20,
  ...
  parameter  Signed = "Yes";
  localparam ISize = (Signed == "No") ? General::clog10(1<<Size):(General::clog10(1<<(Size-1))+1);
  input logic Clock, Button_Up, Button_Reset, Button_Down, Button_Sign;
  ...
  B1 (.Clock(Clock), . Button_Up(Button_Up), .Button_Reset(Button_Reset), .Button_Down(Button_Down),.Button_Sign(Button_Sign), .Data(Data), .SigneBit(SigneBit));
